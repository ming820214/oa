...
//                             $m2->part='人事中心';
					...
					if($m1['position']=='校长'){//校长请假
						if($m1['school'] == '盘锦长颈鹿项目' || $m1['school'] == '盘锦童画创意美术项目' || $m1['school'] =='盘锦BBunion早教项目'){
						    ...
						    if(in_array($m1['school'],['集团总校','盘锦日月兴城校区','盘锦水木清华校区','盘锦大洼一高校区','盘锦天丽家园校区','锦州附中校区','锦州中学校区','阜新市高校区','葫芦岛一高校区','葫芦岛二高校区','阜新实验校区','朝阳二高校区','朝阳建平校区'])){
						        $m2->state='辽西区域审核';
						        $this->text(7,'张玉珠','小文提示：' . $m1['school'] . ' 有请假的申请待审核，请及时查看……');
						    }else if(in_array($m1['school'],['鞍山站前校区','鞍山钢高校区','鞍山八中校区','抚顺一中校区','铁岭一高校区','丹东四中校区','营口盖州一高校区','大石桥丰华伊嘉园校区','鲅鱼圈书香门第校区'])){
						        $m2->state='辽东区域审核';
						        ...
						        $m2->state='吉林区域审核';
						        $this->text(7,'王大鹏','小文提示：' . $m1['school'] . ' 有请假的申请待审核，请及时查看……');
						    }else if(in_array($m1['school'],['齐齐哈尔实验校区','大庆实验中学校区','大庆一中校区','鸡西一中校区','绥化一中校区'])){
						        $m2->state='黑龙江区域审核';
						        $this->text(7,'何亮','小文提示：' . $m1['school'] . ' 有请假的申请待审核，请及时查看……');
						    }
        ...
        if($_POST['yes']){
            ...
            if($m['state']=='辽东区域审核' && session('name')=='张鹏'){
                if($m['class']=='请假' && $m['gong']>2){
                    $d['state']='运营审核';
                    $this->text(7,'张玉珠','小文提示：有请假>2天的申请待审核，请及时查看……');
                }else{
                    $d['state']='集团确认';
                    $this->text(7,'宫婷','小文提示：有校区申请待确认，请及时查看……');
                    $this->text(7,'彭鑫','小文提示：有校区申请待确认，请及时查看……');
...
//             				$this->text(7,'张毅','小文提示：有请假申请待审核，请及时查看……');
            			...
            			/* 王胜鑫替代侯海洋审核教学部请假信息*/
            			  if($m['gong']>2 && $m['state'] == '主管审核'){
            				$d['state']='总裁确认';
            				$this->text(7,'王胜鑫','小文提示：教学部有请假>2天的申请待审核，请及时查看……');
            			}else{
            			    if($m['gong']>2){
            				    
            				    if(in_array($m['school'],['盘锦BBunion早教项目','盘锦童画创意美术项目','盘锦长颈鹿项目','盘锦实验中学校区','盘锦一完中校区'])){
            				        $d['state']='多种经营事业部确认';
            				        $this->text(7,'李明帅','小文提示：' . $m['school'] . ' 有请假>2天的申请待审核，请及时查看……');
            				    }else if(in_array($m['school'],['集团总校','盘锦日月兴城校区','盘锦水木清华校区','盘锦大洼一高校区','盘锦天丽家园校区','锦州附中校区','锦州中学校区','阜新市高校区','葫芦岛一高校区','葫芦岛二高校区','阜新实验校区','朝阳二高校区'])){
            				        $d['state']='辽西区域确认';
            				        $this->text(7,'张玉珠','小文提示：' . $m['school'] . ' 有请假>2天的申请待审核，请及时查看……');
            				    }else if(in_array($m['school'],['鞍山站前校区','鞍山钢高校区','鞍山八中校区','抚顺一中校区','铁岭一高校区','丹东四中校区','营口盖州一高校区','大石桥丰华伊嘉园校区','鲅鱼圈书香门第校区'])){
            				        $d['state']='辽东区域确认';
            				        $this->text(7,'张鹏','小文提示：' . $m['school'] . ' 有请假>2天的申请待审核，请及时查看……');
            				    }else if(in_array($m['school'],['松原江北三中校区','松原宁江实验校区','松原大路校区','松原前郭五中校区','四平一高校区'])){
            				        $d['state']='吉林区域确认';
            				        $this->text(7,'王大鹏','小文提示：' . $m['school'] . ' 有请假>2天的申请待审核，请及时查看……');
            				    }else if(in_array($m['school'],['齐齐哈尔实验校区','大庆实验中学校区','大庆一中校区'])){
            				        $d['state']='黑龙江区域确认';
            				        $this->text(7,'何亮','小文提示：' . $m['school'] . ' 有请假>2天的申请待审核，请及时查看……');
            				    }else{
            				        $d['state']='人事确认';
            				        $this->text(7,'侯海洋','小文提示：有请假>2天的申请待审核，请及时查看……');
            				    }
            					
            			    }
            			} 
            		}
            	}else if($m['gong']>2 && $m['state'] == '运营审核' && session('name')=='张玉珠'){
            	    $d['state']='审核通过';
            	    $this->text(7,'张玉珠','小文提示：' . $m['school'] . '的校长 有请假>2天的申请待审核，请及时查看……');
            	}else{
            		//if(session('name')=='宫婷'){
            		if(session('name')=='彭鑫' || session('name')=='庄宇' || session('name')=='宫婷'){
            			$d['state']='审核通过';
